...
void I3C_MasterSetBaudRate(I3C_Type *base, const i3c_baudrate_hz_t *baudRate_Hz, uint32_t sourceClock_Hz)
{
    ...
    i3cPPLow_Ns = NSEC_PER_SEC / (2UL * freq);

    /* We need ODBAUD generate 2.5MHz or so. */
    if (isODHigh)
    {
        /* odFreq = (2*freq) / (ODBAUD + 2), 1 <= ODBAUD <= 255 */
        div = (2 * freq) / i3cODBaud_HZ;
        div = div < 2UL ? 2UL : div;
        if ((2 * freq / div) > i3cODBaudMax_HZ)
        {
            div++;
        }
        odBaud = div - 2UL;
    ...
    errRate0 = I3C_CalcErrorRatio((NSEC_PER_SEC / (2UL * divEven * i3cOdLow_Ns)), i2cBaud_HZ);

    divOdd   = ((sourceClock_Hz / i2cBaud_HZ) / ((ppBaud + 1UL) * (odBaud + 1UL) - 1UL)) / 2UL;
    divOdd   = divOdd == 0UL ? 1UL : divOdd;
    errRate1 = I3C_CalcErrorRatio((NSEC_PER_SEC / ((2UL * divOdd + 1UL) * i3cOdLow_Ns)), i2cBaud_HZ);
...
void I3C_MasterGetIBIRules(I3C_Type *base, i3c_register_ibi_addr_t *ibiRule)
{
    ...
    for (uint32_t count = 0; count < ARRAY_SIZE(ibiRule->address); count++)
    {
        ibiRule->address[count] = (ruleValue >> (count * I3C_MIBIRULES_ADDR1_SHIFT)) & I3C_MIBIRULES_ADDR0_MASK;
    }
    ...
    while (!state_complete)
    {
        ...
        switch (handle->state)
        {
            ...
            case (uint8_t)kWaitRepeatedStartCompleteState:
                ...
                {
                    handle->state = (uint8_t)kTransferDataState;
                    I3C_MasterDisableInterrupts(base, kI3C_MasterTxReadyFlag);
            ...
            case (uint8_t)kTransferDataState:
                ...
                {
                    if (!handle->isReadTerm && (handle->remainingBytes == rxCount))
                    {
                        handle->isReadTerm  = true;
                        uint32_t mctrlValue = base->MCTRL;
                        mctrlValue &= ~I3C_MCTRL_RDTERM_MASK;
                        base->MCTRL = mctrlValue | I3C_MCTRL_RDTERM(handle->remainingBytes);
                    }
